<div align="center">
    <a href="https://kokkoro.js.org">
      <img src="https://kokkoro.js.org/kokkoro.png" width="200" alt="pixiv: 104321865" />
    </a>
    <h3>kokkoro</h3>
    <small> &gt; とある咕咕の QQ 机器人框架 &lt; </small>
</div>

---

[![priconne redive](https://img.shields.io/badge/%E3%83%97%E3%83%AA%E3%83%B3%E3%82%BB%E3%82%B9%E3%82%B3%E3%83%8D%E3%82%AF%E3%83%88-Re%3ADive-F55291?style=flat-square&labelColor=FAFAFA&logo=data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAMAAABEpIrGAAAABGdBTUEAALGPC%2FxhBQAAACBjSFJNAAB6JgAAgIQAAPoAAACA6AAAdTAAAOpgAAA6mAAAF3CculE8AAABSlBMVEX%2F%2F%2F%%2F%2F%2F%2BQAc1iAAAAbHRSTlMAAAtOkbLJzLeYWxYQjev3oyLq%2BnqLtImwT4QF%2BKlKQ44rirqMyAF0KSFGeJKAoUEg8LOivbE1gxMEb1OP1cE9FREGGOPCNxQS5JwcB2x52JedhlItmYLAdSytDDMuZaXLxZaq%2Fj4fcKvZzX%2BVYCqxAAAAAWJLR0QAiAUdSAAAAAd0SU1FB%2BUHAgwRA1G%2Bv8QAAAFESURBVDjLY2CgC2CEAiZmFlY2dg5OLm6YCIoCHl6%2BHCjgFxDEVMAllIMEhEXQFYjmoAExVAXiORhAAlmBZA4WIIVQIC0DE5SVk1eAq1CEK1CCCSmDLFaB8VRhCtSEYULqIAUacCM0oQq04CKsIAXaCFdAFehAuBy6ejL6jIwGhka6xhARE6gCU7i3zMwtLK2sgQwbsJAtVIEdmGcPDjsHRycQ5QwWcoEqcAXz3MAKHN3BlAdYyBOqQAzM8wKKe2s7%2BPiCVPqBhfyhCgIgTgq0DuILZmQMEQq1DoOIhEMVMMP95QoyHh5sObxQBT5wkQiQgkg4NwQW1IEwkSiUgPKHx0U0XE9gTKwunOOGiO44bNHthZxg4jHlE1CTXCC6fKITWqKNQJVPwkz2GskpiPBIxZIvGBnT0jNUMzOzJLIV0TIOjQEAe%2FGzgoDvmDIAAAAldEVYdGRhdGU6Y3JlYXRlADIwMjEtMDctMDJUMTI6MTc6MDMrMDM6MDArdontAAAAJXRFWHRkYXRlOm1vZGlmeQAyMDIxLTA3LTAyVDEyOjE3OjAzKzAzOjAwWisxUQAAAABJRU5ErkJggg%3D%3D)](https://priconne-redive.jp/)
[![qq group](https://img.shields.io/badge/group-1030982964-12B7F5?style=flat-square&labelColor=FAFAFA&logo=data:image/png;base64,&logoColor=000000)](https://jq.qq.com/?_wv=1027&k=3hcWCnhq)
[![npm package](https://img.shields.io/npm/v/kokkoro?color=57B497&label=kokkoro&style=flat-square&labelColor=FAFAFA&logo=npm)](https://www.npmjs.com/package/kokkoro)
[![node engine](https://img.shields.io/node/v/kokkoro?color=339933&style=flat-square&labelColor=FAFAFA&logo=Node.js)](https://nodejs.org)

> 本项目是一个基于 [Amesu](https://github.com/xueelf/amesu) SDK，使用 [TypeScript](https://www.typescriptlang.org/) 语言开发的 [QQ](https://im.qq.com/) 机器人框架。

## 介绍

> 腾讯近期将开放官方 Bot API，协议库时代已成为历史。kokkoro 代码全被移除，待正式上线时开始进行重构。

原项目 yumemi_bot 最初为个人自用，主要围绕游戏 [公主连结☆Re:Dive](https://priconne-redive.jp/) 开发相关功能。因代码严重耦合不利于维护，使用 ts 分离重构为插件一对多形式的框架。

## 使用

> 现在是幻想时间，下列的伪代码是理想中的开发效果，可能会在未来发生变化。

你可以使用 CLI 来快速构建项目：

```shell
npx @kokkoro/cli init
```

```shell
✔ Project name: … <your-project-name>
✔ Server Port? … <your-server-port>

Scaffolding project in ./<your-project-name>...
Done.
```

### 目录结构

```tex
.
├─ db                 数据库
├─ logs               日志
├─ plugins            插件
├─ main.js            主程序
└─ kokkoro.json       配置文件
```

安装好依赖文件后使用 `npm start` 或 `node main.js` 启动项目必须要配置 `kokkoro.json` 文件。

### 配置项

```json
{
  // web 服务
  "server": {
    // 端口号
    "port": 2333,
    // 域名
    "domain": null
  },
  // 日志等级
  "logLevel": "info",
  // bot 信息，可添加多个
  "bots": [
    {
      "id": "BotAppID",
      "token": "BotToken"
    }
  ]
}
```

若要开发插件，可以在项目根目录使用下列命令：

```shell
npx @kokkoro/cli create
```

```shell
✔ Plugin name: … <your-plugin-name>
✔ Add JSX Support? … No / Yes

Scaffolding plugin in ./<your-plugin-name>...
Done.
```

目前有两种设计思路，暂时还没确定下来。

### 类（支持 js、jsx、ts、tsx）

```tex
plugins/
└─ demo/                 插件名
   ├─ index.js
   └─ package.json
```

```javascript
// index.js
import { Plugin } from '@kokkoro/core';

export const plugin = new Plugin('demo');

plugin
  .command(/^测试$/)
  .action(() => 'hello world');

plugin
  .event('MESSAGES')
  .action(ctx => {
    console.log(`收到了消息: ${ctx.message}`);
  });
```

### 装饰器（仅支持 ts、tsx）

```tex
plugins/
└─ demo/                 插件名
   ├─ src
   │  └─ index.ts
   ├─ package.json
   └─ tsconfig.json
```

```typescript
import { Command, Event, Plugin } from '@kokkoro/core';

@Plugin({ name: 'demo' })
export class DemoPlugin {
  constructor() {}

  @Command(/^测试$/)
  test() {
    return 'hello world';
  }

  @Event('MESSAGES')
  onMessage(ctx: any) {
    console.log(`收到了消息: ${ctx.message}`);
  }
}
```

### JSX 支持

```jsx
<>
  <at id={}/>
  <text>hello world</text>
  <image url="" />
</>
```

这两种插件方案都是很早之前就实现过的，我个人更倾向于 decorators，但是这一特性刚进入 [stage3](https://github.com/tc39/proposal-decorators) 没多久，还有很长的路要走。
