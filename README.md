...
[![npm package](https://img.shields.io/npm/v/kokkoro?color=57B497&label=kokkoro&style=flat-square&labelColor=FAFAFA&logo=npm)](https://www.npmjs.com/package/kokkoro)
...
**鉴于腾讯对机器人的封杀态度，这个项目以后不会再更新了**

各大社交平台，Discard、Telegram、Twitter、Facebook、Line... 官方都有为开发者提供相应的 SDK，就算是在国内，钉钉和飞书也有提供，就腾讯特殊，喜欢封杀。

kokkoro 从最初到现在也维护了快三年了，有很多不舍，但从今天起，也该放下了。有业余的时间写点别的不好么？为什么非要去写 **QQ 机器人** 这种鬼东西？

近期账号频繁掉线，想着实在不行就迁移到 QQ 频道，虽然现在还不完善但是起码稳定。非要让我用腾讯文档登录查看就算了，结果捣鼓了半天连文档都打不开，提示该项目负责人已离职，无权查看，呵...

小趣闻：大学时期（2018）有幸在臭名昭著的盗版 Steam 公司做过一段时间客服（面试的时候并不知道），想着赚点零花钱。公司每周都会收到工商局的传单，QQ 群几乎活不过一个星期就会被封。但是客服有专属的 **营销 QQ 客户端**，能设置自定义回复，而且不管怎么打广告被举报都不会被封号，吃相真难看。

## 介绍

> 目前 kokkoro 处于 alpha 状态，不过她已经适用于开箱即用，极少部分 API 和配置项可能会在未来版本中发生变化。

原项目 yumemi_bot 最初为个人自用 bot，主要围绕 [公主连结☆Re:Dive](https://priconne-redive.jp/) 开发相关功能，因代码严重耦合不利于维护，使用 ts 分离重构为插件一对多形式的框架。

- [x] 多群插件管理，所有插件针对不同群聊均支持参数自定义
- [x] 支持扫码登录，能有效避免因操作不当导致账号风控、掉线等问题
- [x] 友好的脚手架，无需手动修改配置文件，全由命令自动构建
- [x] 账号同步登录，可添加多个 bot 自由管理
- [x] web 可扩展性，每个插件均支持独立的 web 页面及路由服务
