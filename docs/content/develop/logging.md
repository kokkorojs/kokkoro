...
如果需要在 Linux 服务器上运行机器人并将日志保存到文件，参阅 [部署](/guide/deployment)。

## 记录插件日志 {#write-logs}

`useLogger()` 返回当前插件的日志记录器。它必须在插件模块加载期间调用，通常放在模块顶层。将返回值保存为 `logger` 后，插件中的其他代码都可以使用它。
...
Kokkoro 只能在加载插件模块时判断日志记录器属于哪个插件，因此不允许在 `PluginSetup` 内调用 `useLogger()`。模块加载与插件挂载之间的关系见 [插件生命周期](/develop/lifecycle)。
